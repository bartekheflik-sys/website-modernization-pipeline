...
- LOGO: ${primaryLogo ? `Use this EXACT URL for the logo: ${primaryLogo}. DO NOT use a placeholder or a generic icon. It is mandatory for the navigation and footer.` : 'There is no graphic logo image on the legacy website. You MUST NOT render an <img> tag for the logo in the header or footer (doing so causes broken image placeholders). Instead, render a premium, high-end text-logo/brandmark using elegant typography (e.g., tracking-wider, uppercase, serif/sans-serif combination) paired with a beautiful Lucide icon matching the business type (e.g., Lucide "Sprout", "Flower", or "Sparkles" for a spa/massage business) inside a stylized CSS circle/badge. This ensures it looks like a custom boutique luxury brand mark with zero broken image links.'}
