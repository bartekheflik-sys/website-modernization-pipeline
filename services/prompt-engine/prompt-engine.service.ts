import { fetchAnalysisResult, saveGeneratedPrompt, fetchProjectAssets, fetchProjectPages } from './storage/prompt.storage';
...
export async function generatePromptForProject(projectId: string): Promise<{ status: string; result?: GeneratedPromptOutput; error?: string }> {
  try {
    ...
    console.log(`[Prompt Engine] Step 3 analysis, ${assets.length} assets, and ${pages.length} pages fetched successfully.`);

    // STEP 2: Generate the full deterministic Lovable prompt
    const promptOutput = generateLovablePrompt(analysisJson, assets, pages);
